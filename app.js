...
function getDefaultMenuItems() {
  return [
    ...
    },
    {
      ...
      category: 'appetizers',
      price: 120,
      description: 'Crispy golden potato fries with a sprinkle of salt',
    ...
    },
    {
      ...
      category: 'main-course',
      price: 180,
      description: 'Juicy vegetable patty burger with lettuce and tomato',
    ...
    },
    {
      ...
      category: 'main-course',
      price: 140,
      description: 'Crispy South Indian rice crepe served with chutney',
...
function displayCartItems() {
  ...
  cart.forEach(item => {
    ...
    cartItemDiv.innerHTML = `
      <img src="${item.image}" alt="${item.name}" class="cart-item-img" onerror="this.src='https://via.placeholder.com/60x60?text=Food'">
...
function setupAddItemForm() {
  ...
  if (saveItemBtn) {
    saveItemBtn.addEventListener('click', function() {
      ...
      // Generate next serial number for ID
      let nextId = 1;
      if (menuItems.length > 0) {
        // Only consider numeric IDs
        const numericIds = menuItems.map(item => typeof item.id === 'number' ? item.id : parseInt(item.id)).filter(id => !isNaN(id));
        nextId = Math.max(...numericIds, 0) + 1;
      }
      const newItem = {
        id: nextId,
